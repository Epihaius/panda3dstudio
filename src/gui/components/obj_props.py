...
class TagDialog(wx.Dialog):
    ...
    def __init__(self, parent, tags):
        ...
        tag_sizer.Add(wx.StaticText(tag_panel, -1, "Key"), 0, wx.ALIGN_CENTER)
        tag_sizer.Add(wx.StaticText(tag_panel, -1, "Value"), 0, wx.ALIGN_CENTER)
        tag_sizer.AddStretchSpacer()

        get_commit_handler = lambda key: lambda evt: self.__commit(key)
        get_remove_handler = lambda key, i: lambda evt: self.__remove_tag(key, i)
        ...
        for key, value in tags.iteritems():
            ...
            handler = get_remove_handler(key, index)
    ...
    def __clear(self, event):
        ...
        if answer == wx.OK:
            ...
            for index in range(len(self._tags)):
                self._tag_sizer.Remove(index + 3)
    ...
    def __add_tag(self, event):
        ...
        handler = lambda evt: self.__remove_tag(key, index)
    ...
    def __remove_tag(self, key, index):
