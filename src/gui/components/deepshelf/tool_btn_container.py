...
class ToolButtonContainer(ButtonContainer):
    ...
    def init(cls, btn_bitmap_name):
        ...
        cls._mgr.accept("add_tool_button_props", cls.__addButtonProps)
        cls._mgr.accept("get_tool_button_data", cls.__getButtonData)
        cls._mgr.accept("toggle_tool_button", cls.__toggleButton)

    @classmethod
    def __addButtonProps(cls, btn_id, btn_props):
    ...
    @classmethod
    def removeToggleButton(cls, button):
    ...
    @classmethod
    def clearToggleButtons(cls):

        cls._toggle_btns = {}

    def insertToolButtons(self, btn_data, x=100000):

        for btn_props in btn_data:
            self.insertToolButton(x, props=btn_props)
            x += self._tool_btn_space

    def insertToolButton(self, mouse_x, button=None, props=None):
        ...
        if button:
            ...
            if button in self._btns:
                ...
                if old_index < index:
                    # print "Adjusting new index."
                    index -= 1
                    x -= self._tool_btn_space

                if old_index == index:
                    # print "Attempt to drop button onto itself!"
                    return

            button.get_shelf().removeToolButton(button, destroy=False)
            button.setShelf(self._proxy)
            button.set_x(x)

            data = self._panel.getShelfDataValue(self._id, "tools")
            data.insert(index, button.getProps())
...
# print "Created button:", button

            if btn_id in self._toggle_btns:

                self._toggle_btns[btn_id].append(button)
# print "There are now", len(self._toggle_btns[btn_id]), btn_id, "buttons."

                if self._toggle_btns[btn_id][0].isToggledOn():
                    button.toggle()

            else:

                self._toggle_btns[btn_id] = [button]
# print "Added 1st", btn_id, "button."
    ...
    def removeToolButton(self, button, destroy=True):
        ...
        btn_data = self._panel.getShelfDataValue(self._id, "tools")
        btn_props = button.getProps()
        btn_data.remove(btn_props)
        button.set_cut(False)
        self._panel.releaseCutButton(button)

        if destroy:
            self.removeToggleButton(button)
    ...
    def drop_tool_buttons(self, x, buttons):
        ...
        if len(set(buttons + self._btns)) > self._max_btn_count["tool"]:
            ...
            return self._panel.relocateShelfContents(buttons, "tool")
        ...
        if len(buttons) > 1 and buttons[0] in self._btns:
            ...
            for button in buttons:
                self.removeToolButton(button, destroy=False)
        ...
        for button in buttons:
            if self.insertToolButton(x, button):
                x += self._tool_btn_space
