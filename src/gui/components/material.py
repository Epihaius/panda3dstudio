...
class MaterialPanel(Panel):
    ...
    def __init__(self, parent, focus_receiver=None):
        ...
        hist_section = self.add_section("history", "History")
        sizer = hist_section.get_client_sizer()
        ...
        btn = PanelButton(self, section, btn_sizer, bitmaps, label, "Load material library",
                          lambda: None, sizer_args, focus_receiver=focus_receiver)

        label = "Save"
        bitmaps = PanelButton.create_button_bitmaps(
            "*%s" % label, bitmap_paths)
        btn = PanelButton(self, section, btn_sizer, bitmaps, label, "Save material library",
                          lambda: None, sizer_args, focus_receiver=focus_receiver)
        ...
        Mgr.add_app_updater(
            "new_material", self.__update_new_material, ["select"])
        ...
        Mgr.add_app_updater(
            "new_tex_layer", self.__update_new_layer, ["select"])
...
class MaterialToolbar(Toolbar):
    ...
    def __init__(self, parent, pos, width):
        ...
        btn = Button(self, bitmaps, "", tooltip_label,
                     self.__clear_all_textures)
        sizer.Add(btn, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 2)
        ...
        def get_command(color_type):
            ...
            def set_color_type():
                ...
                Mgr.update_remotely(
                    "ready_material_color_selection", color_type)

            return set_color_type

        color_types = ("diffuse", "ambient", "emissive", "specular", "alpha")
        labels = ("Diffuse", "Ambient", "Emissive",
                  "Specular", "Transp./opacity")
        ...
        command = lambda: Mgr.update_remotely(
            "selected_obj_mat_prop", self._color_type)
        ...
        command = lambda: Mgr.update_remotely(
            "selected_obj_mat_prop", "shininess")
        btn = Button(self, bitmaps, "", tooltip_label, command)
        sizer.Add(btn, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 2)
        ...
        command = lambda: Mgr.update_remotely(
            "selected_obj_prop", "material", None)
        btn = Button(self, bitmaps, "", tooltip_label, command)
        sizer.Add(btn, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 2)
