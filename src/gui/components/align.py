...
class AlignTypeComboBox(ToolbarComboBox):
    ...
    def __init__(self, toolbar):
        ...
        def add_target_type_entry(target_type):

            def set_target_type():

                Mgr.update_remotely("object_alignment", "pick_target", target_type)

            self.add_item(target_type, target_type, set_target_type, select_initial=False)
        ...
        target_types = ("view", "object", "obj_axis_point", "surface")
...
class SnapAlignToolbar(Toolbar):
    ...
    def setup(self):
        ...
        add_state("alignment_target_picking_mode", -10, enter_picking_mode, exit_picking_mode)
        add_state("surface_alignment_mode", -10, enter_align_mode, exit_align_mode)
...
class AlignmentDialog(Dialog):
    ...
    def __init__(self, target_type, obj_name=""):
        ...
        if target_type == "surface":
            ...
            if target_type == "obj_axis_plane":
                group_title = "Target plane"
            else:
                group_title = "Target direction"

            subgroup = DialogWidgetGroup(group, group_title)
            borders = (5, 5, 5, 5)
        ...
        if obj_lvl != "normal" and target_type != "obj_axis_point":
