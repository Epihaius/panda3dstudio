...
class EditableGeomManager(BaseObject, ObjPropDefaultsManager):
    ...
    def __update_object_level(self):
        ...
        if self._sel_before_hist_change:
            ...
            for obj_id in self._sel_before_hist_change:

                model = Mgr.get("object", obj_id)
    ...
    def __check_selection(self):
        ...
        sel_after_hist_change = set([obj.get_id()
        ...
        if sel_after_hist_change == self._sel_before_hist_change:
            ...
            for obj_id in sel_after_hist_change:

                obj = Mgr.get("object", obj_id)

                if obj.get_geom_type() != "editable_geom":
                    set_sublvl = False
                    break
    ...
    def __start_selection_check(self):
        ...
        self._sel_before_hist_change = set(
            [obj.get_id() for obj in Mgr.get("selection", "top")])
...
class EditableGeom(BaseObject):
    ...
    def __init__(self, model, geom_data_obj):
        ...
        self._model = model
        self._geom_data_obj = geom_data_obj if geom_data_obj else GeomDataObject(
            self)
